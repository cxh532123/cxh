...
class Resource:
    ...
    # 不同关卡的飞机
    HERO_IMG = [
        "images/hero_planes/hp01_01.png",
        "images/hero_planes/hp02_01.png",
        "images/hero_planes/hp03_01.png",
        "images/hero_planes/hp04_01.png",
        "images/hero_planes/hp04_04.png",
    ]
...
class Engine:
    ...
    def __score_upgrade(self):
        """判断关卡的操作"""
        # 是否过关
        if Resource.HERO_SCORE in [5, 10, 15, 20]:
            return True
        return False

    def start(self):
        """开始游戏的方法"""
        while self.step <= 5: # 循环关卡：升级的主要的控制
            self.step += 1
            # 播放BGM
            # Resource.play_bgm()

            # 过关动画
            # TODO 每次新的一关开始加载资源之前，先播放一些过关动画

            # 创建背景精灵对象
            bg1 = BackgroundSprite(Resource.BACKGROUND_IMG[self.step-1],
                                   position={"x": 0, "y": 0},
                                   speed={"x": 0, "y": 2})
            bg2 = BackgroundSprite(Resource.BACKGROUND_IMG[self.step-1],
            ...
            # 创建英雄飞机
            hero = HeroPlane(Resource.HERO_IMG[self.step-1], position={"x": 200, "y": 500},
                             speed={"x": 3+self.step, "y": 5+self.step}, blood=1000)
            # 将英雄飞机添加到精灵组
            ...
            while True:
                ...
                # 调用判断关卡
                res = self.__score_upgrade()
                if res:
                    # 清空上一关的所有精灵组中的精灵
                    self.common_group.empty()
                ...
                Resource.print_text(0, 20, f"HERO Y POINT:{hero.rect.y}") # 英雄飞机Y坐标
                Resource.print_text(0, 40, f"ENEMIES COUNT:{len(self.enemy_group)}") # 屏幕上敌方飞机数量
                Resource.print_text(0, 60, f"ESCAPE COUNT:{Resource.ESCAPE_COUNT}") # 逃逸的地方飞机数量
                Resource.print_text(0, 80, f"PLAYER SCORE:{Resource.HERO_SCORE}") # 玩家积分
                Resource.print_text(0, 100, f"HERO BLOOD:{hero.blood}") # 飞机血量
